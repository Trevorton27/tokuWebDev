...
export async function createAndSendInvite(
  ...
  logger.info('consultation: invite created', { email, source });
  ...
  const { error } = await resend.emails.send({
    ...
    html: buildInviteHtml(greeting, bookingUrl),
    text: buildInviteText(greeting, bookingUrl),
  });

  if (error) {
    logger.error('consultation: failed to send invite email', { email, error });
  } else {
    logger.info('consultation: invite email sent', { email });
  }
}

function buildInviteHtml(greeting: string, bookingUrl: string): string {
  return `<!DOCTYPE html>
<html lang="en">
        ...
        <tr>
          <td style="padding:36px 40px;">
            ...
            <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
              Thank you for completing the Signal Works Design assessment. The next step is a
              <strong>free 30-minute consultation</strong> where we'll review your results together,
              discuss your roadmap, and confirm which course is the right fit.
            </p>
...
function buildInviteText(greeting: string, bookingUrl: string): string {
  return [
    greeting,
    '',
    'Thank you for completing the Signal Works Design assessment.',
    '',
    "The next step is a free 30-minute consultation where we'll review your results,",
    'discuss your personalized roadmap, and confirm which course is the right fit.',
    '',
