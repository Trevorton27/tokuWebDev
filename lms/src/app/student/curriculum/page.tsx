  ...
  useEffect(() => {
    async function fetchRoadmap() {
      try {
        ...
        const errorMessage = err.response?.data?.error === 'No roadmap document assigned'
          ? 'No roadmap document assigned'
          : err.response?.data?.error || t('student.failedToLoadCurriculum');
          ...
          <div className="bg-white dark:bg-dark-card rounded-xl shadow-md p-12 border border-gray-200 dark:border-dark-border">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">
                {t('student.noRoadmapAssigned')}
              </h2>
              <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                {t('student.noRoadmapDesc')}
              </p>
              <div className="flex justify-center gap-4">
                ...
                <Link href="/student/roadmap" className="px-6 py-3 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
                  {t('student.viewInteractiveRoadmap')}
                </Link>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-white dark:bg-dark-card rounded-xl shadow-md p-8 border border-gray-200 dark:border-dark-border">
            <div className="text-center py-12">
              ...
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">{t('student.failedToLoadCurriculum')}</h2>
        ...
        <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2">
              {roadmap?.title || t('student.curriculumRoadmap')}
            </h1>
            <p className="text-gray-600 dark:text-gray-300">{t('student.curriculumDesc')}</p>
          </div>
          {isAdmin && editUrl && (
        ...
        <div className="bg-white dark:bg-dark-card rounded-xl shadow-md border border-gray-200 dark:border-dark-border overflow-hidden">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-8">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-sm font-semibold text-blue-900 dark:text-blue-200 mb-1">{t('student.learningRoadmap')}</h3>
                    <p className="text-sm text-blue-800 dark:text-blue-300">
                      {t('student.learningRoadmapDesc')}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-gray-50 dark:bg-dark-surface px-8 py-4 border-t border-gray-200 dark:border-dark-border">
            <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
              <p>{t('student.lastUpdated')}: {new Date().toLocaleDateString()}</p>
        ...
        <div className="mt-8 grid md:grid-cols-2 gap-6">
          <Link href="/student" className="bg-white dark:bg-dark-card rounded-lg shadow-md p-6 border border-gray-200 dark:border-dark-border hover:shadow-lg transition-shadow">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-white">{t('student.dashboard')}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">{t('student.viewLearningPath')}</p>
              </div>
            </div>
          </Link>
          ...
          <Link href="/student/roadmap" className="bg-white dark:bg-dark-card rounded-lg shadow-md p-6 border border-gray-200 dark:border-dark-border hover:shadow-lg transition-shadow">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-white">{t('student.roadmap')}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">{t('student.trackProgressByTask')}</p>
              </div>
            </div>
          </Link>
        </div>
